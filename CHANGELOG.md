...
### 🚀 X 课程开发计划 v1 + Phase 1 首个实验代码包

启动 X 课程的**开发执行阶段**，采用「5 阶段流水线 + 2 用户确认门禁」模式：

**新建文档（1篇）**：
- ✅ `X课程开发计划-v1.md` — 五阶段流水线（实验代码→课时大纲→教学设计+实训手册→PPT内容设计→PPT开发），含交付物总览（144 份）、Phase 1 实验代码清单（32 课时全预览）、批次推进与并行优化策略

**新建实验代码包（1个）**：
- ✅ `实验代码/U1-L1_K1连接与信息/` — Phase 1 首个代码包，含 `main.py`（核心脚本）+ `README.md`（实验说明/前置知识/步骤/预期结果/思考题/教师建议）+ `requirements.txt`

**X 课程开发计划核心要点**：
- 5 阶段：Phase 1 实验代码(32包) → Phase 2 课时大纲(8份) → [确认①] → Phase 3 教学设计+实训手册(64份) → Phase 4 PPT内容设计(8份) → [确认②] → Phase 5 PPT(32份)
- 存储于 `vocational-planning/实验代码/`、`课时大纲/`、`教学设计/`、`实训手册/`、`PPT内容设计/`、`PPT/`
- 前 16 课时（U1-U4）优先完成，支持"前 16 独立开课"设计
- 下一节点：Phase 1 U1-L2（ROS2 通信基础）
...
X 课程（代号，正式名称待定）是面向职业院校的 **BoosterOS SDK 实训实操基础课**，位于课程体系链路「通识理论 → 数采课程 → **X 课程** → Z 课程(AgentFramework)」的第三环节。

**新建文档（2篇）**：
- ✅ `X课程大纲-v3-clean.md` — 精华版大纲（250 行），面向外部阅读
- ✅ `X课程大纲-v3.md` — 完整版大纲（400 行），含概念分布总表、SDK 覆盖检查、ROS2 知识线、与 Z 课程衔接、待定事项

**课程核心特征**：
- 8 单元 × 32 课时（U1-U8），前 16 课时可独立开课
- 双主线设计：具身智能知识线 + 编程/应用线
- 36 个概念全覆盖（全部可实操验证），D 类具身认知概念全部删除
- SDK 能力 11/11 全覆盖
- 每个单元产出轻量项目：视觉调试工具 → 方位检测器 → 追球控制器 → 状态监视器 → 迎宾 Agent → 动作部署器 → 多模态机器人 → 主题实战
- U8「任务集市」模式：3 个可选主题（智能踢球/智能导览/传感器数据采集器）

### 🔗 连锁更新

- ✅ `CHANGELOG.md` — v1.13
- ✅ `INDEX.md` — 新增 X 课程关键词条目
- ✅ `05-internal-materials/_README.md` — 更新 vocational-planning 统计
