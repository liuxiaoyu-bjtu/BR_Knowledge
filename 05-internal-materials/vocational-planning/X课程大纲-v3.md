...
1. **衔接前置**：把通识理论课中的抽象概念（计算机视觉、运动控制、传感器、语音等）通过 K1 实操具象化**衔接前置（非必须前置）**：把“具身通识理论课（任意专业必修）”中的抽象概念（计算机视觉、运动控制、传感器、语音等）通过 K1 实操具象化
...
- **概念精筛**：只引入可实操验证的概念（D 类具身认知概念全部删除）
- **前后可拆**：前 16 课时（U1-U4）可独立开课，后 16 课时（U5-U8）可续接

### 1.4 先修假设

- 学生已完成「具身智能通识 / AI 通识」等理论课程（了解基本概念词汇）
- 学生已具备 Python 基础、Linux 基础操作能力
...
![[Pasted image 20260811141102.png]]
...
<table border="1" style="border-collapse: collapse; text-align: center; width: 100%;">
  <thead>
    ...
    <tr style="background-color: #f2f2f2;">
      <th colspan="4">前16课时（基础感知与控制）</th>
      <th colspan="4">后16课时（交互与集成实践）</th>
    </tr>
  </thead>
  <tbody>
    <!-- 第 2 行：单元编号 -->
    ...
    <tr style="background-color: #fafafa; font-weight: bold;">
      <td colspan="4">整合 → P1 视觉踢球</td>
      <td colspan="4">整合 → P2 智能导览</td>
    </tr>
  </tbody>
</table>
```
...
flowchart LR
    ...
    subgraph S1 ["前16课时（基础感知与控制）"]
        ...
        subgraph S1_Units [" "]
            direction LR
            U1["U1<br/>SDK入门与ROS2<br/>4课时<br/><b>视觉调试工具</b>"]
...
ROS2 概念在 U1 集中建立基础，后续单元按需延伸：

```
U1（集中）：节点 / 话题 / 发布 / 订阅 / 消息 + ros2 topic CLI 工具
    │
    ├──→ U4（延伸）：定时器 + 参数深入（ros2 param get/set）
    │
    ├──→ U5（延伸）：服务（请求-响应）+ 动作Action（带反馈任务）
    │
    └──→ U7（延伸）：Launch File（一键多节点启动）
```
...
| 概念理解 | 20% | 答辩时随机抽2个本组用到的概念，用实际案例解释（抽查表：36概念） |
...
> 共 36 个概念，全部可实操验证。每单元 2-7 个核心概念（需讲透），0-2 个轻量概念（一句话/5分钟）。

| 单元 | 核心概念（需讲透） | 轻量概念（一句话） | 概念数 |
|---|---|---|---|
| **U1** | SDK / 节点 / 话题 / 发布 / 订阅 / 消息 / 终端调试 | 事件循环 / 回调 / 关节状态 / 参数(轻量引入) | 11 |
| **U2** | YOLO检测 / 置信度 / 检测框 / 归一化 / 主动感知 | 图像坐标系 / CvBridge | 7 |
| **U3** | 偏差 / P控制 / 感知-行动耦合 / 闭环vs开环 / 低通滤波 | Pitch / Yaw | 7 |
| **U4** | TF坐标变换 / 多传感器融合 | 参数(深入) / 定时器 | 4 |
| **U5** | QoS / 服务 / 动作Action / 状态机 | — | 4 |
| **U6** | — | 行为树 | 1 |
| **U7** | Launch File | — | 1 |
| **U8** | 全部 36 概念回顾与抽查 | — | — |
...
| 来源 | 编码 | 数量 |
|---|---|---|
| 知识性概念 | A01-A06, A08-A11 | 11 |
| 算法策略类 | B01-B05, B08, B09 | 7 |
| 工程架构类 | C01-C09 | 9 |
| 具身认知类 | — | 0（全删） |
| 实操技能类 | E01-E07 | 7 |
| 其他（行为树B06） | B06 | 1 |
...
| SDK 能力域 | 接口/API | 所在单元 |
|---|---|---|
| 连接与信息 | `BoosterRobot()`, `robot_info`, `list_joints()`, `list_actions()` | U1 |
| 模式与状态 | `set_mode()`, `get_mode()`, `get_fall_down_state()`, `get_battery()` | U1, U4 |
| 图像采集 | `get_image()`, `subscribe_image()` | U1, U2 |
| 目标检测 | `Detection.detect()` | U2 |
| 运动控制 | `set_velocity()`, `set_joints()`, `set_head_angle()`, `set_gait()` | U3 |
| IMU/里程计 | `get_imu()`, `get_odom()` | U4 |
| 语音 ASR/TTS | `recognize_stream()`, `play_stream()`, `chat()` | U5 |
| LLM 对话 | `ChatConfig(system_prompt=...)` | U5 |
| 示教与轨迹 | `hand_guiding_manager`, `execute_trajectory()`, `do_action()` | U6 |
| 踢球 | `SoccerKickManager` | U8 |
| **覆盖率** | **11/11 能力域** | |
...
| 类别          | 要求                                            |
| ----------- | --------------------------------------------- |
| 机器人         | Booster K1（2-3人/组）                            |
| 学生电脑        | Window\Linux均可                                |
| 软件          | BooterStudio                                  |
| 网络          | 学生电脑与 K1 同一局域网（WiFi或者有线直连）                    |
| 场景道具        | 足球、简易挡板（调参用）                                  |


---

## 九、与后续 Z 课程（AgentFramework）的衔接

| X 课程(本课程)               | Z 课程(AgentFramework)         |
| ----------------------- | ---------------------------- |
| 面向 K1 SDK 单设备调用         | 面向 AgentFramework 多Agent协同开发 |
| 每次操作一个 K1 能力            | 编排多个 Agent 协同完成任务            |
| 状态机(B05)概念引入            | 行为树(B06) + Agent状态流转深入       |
| `do_action()` 调用单个预定义动作 | 技能(Skill)的注册、发现、组合与动态选择      |
| 手工编排动作序列                | 框架驱动的任务规划与执行                 |
| 掌握 SDK 个体能力             | 掌握框架构建复杂应用                   |

---

## 十、待定事项

- [ ] **课程正式名称**：当前代号 X，最终名称待定
- [ ] **U8 主题扩展**：当前3个可选主题是否足够？需根据试点反馈调整
- [ ] **U6 是否需要新增内容**：2课时中"动作编排"部分如果时间过紧，可考虑仅保留部署体验，编排移到 U7 或 U8
