...
- `param_schema_path`：参数 schema 文件路径，用于在APP中配置存储Agent需要的参数信息。详情请参考 参数系统 章节。
...
参数系统依赖 `AgentFeatures.param_schema_path` 指定的 `schema.json` 文件。schema 用来描述 App 参数配置页的字段、展示文本、默认值和校验规则，也用于框架推断参数类型。
...
schema 顶层是一个 JSON 对象，包含国际化文本表和字段列表：
...
- `lang`：多语言文本表。通常包含 `zh` 和 `en`，字段中的 `nameI18nKey`、`descriptionI18nKey`、`unitI18nKey`、`placeholderI18nKey`、`trueTextI18nKey`、`falseTextI18nKey` 会引用这里的 key。

- `fields`：参数配置页的字段数组，按数组顺序展示。除 `divider` 外，每个字段都会通过 `key` 对应一个 ROS 2 参数名。

**通用字段属性**

|**属性**|**是否必填**|**说明**|
|---|---|---|
|`fieldType`|是|字段类型：`boolean`、`string`、`integer`、`float`、`select` 或 `divider`|
|`key`|除 `divider` 外是|ROS 2 参数名，也是代码读写参数时使用的名称|
|`nameI18nKey` / `name`|建议|展示名称。优先使用 `nameI18nKey`，避免硬编码界面文案|
|`descriptionI18nKey` / `description`|否|参数说明文案|
|`defaultValue`|否|默认值，类型必须和字段类型、`valueType`、`multiple` 匹配|

**各字段类型的专有属性**

|**字段类型**|**专有属性**|
|---|---|
|`string`|`placeholderI18nKey` / `placeholder`，`defaultValue` 是字符串|
|`integer`|`unitI18nKey` / `unit`，`lowerBound`，`upperBound`，`placeholder`，`defaultValue` 是整数|
|`float`|同 `integer`，但 `defaultValue`、边界和选项值可以是浮点数|
|`boolean`|`trueTextI18nKey`，`falseTextI18nKey`，`defaultValue` 是布尔值|
|`select`|`valueType: "integer" | "float" | "string"`，`multiple`，`options`，`defaultValue`|
|`divider`|`nameI18nKey` / `name`，仅用于视觉分组，不创建参数|

`select.options` 是选项数组，每个选项包含展示文案和实际写入参数的值：

```JSON
{ "nameI18nKey": "low", "value": 1 }
```

`select` 的 `valueType` 决定 `options[].value` 和 `defaultValue` 的值类型。`multiple` 为 `false` 或省略时，`defaultValue` 是单个值；`multiple` 为 `true` 时，`defaultValue` 必须是数组。
...
|`fieldType: "select"`，`valueType: "integer"`，`multiple: false`|`ENUM_INT`|
|`fieldType: "select"`，`valueType: "float"`，`multiple: false`|`ENUM_FLOAT`|
|`fieldType: "select"`，`valueType: "string"`，`multiple: false`|`ENUM_STRING`|
|`fieldType: "select"`，`valueType: "integer"`，`multiple: true`|`ENUM_MULTI_INT`|
|`fieldType: "select"`，`valueType: "float"`，`multiple: true`|`ENUM_MULTI_FLOAT`|
|`fieldType: "select"`，`valueType: "string"`，`multiple: true`|`ENUM_MULTI_STRING`|
...
```JSON
{
  "lang": {
    ...
    "en": {
      ...
      "enabledDesc": "Whether to enable this feature",
      "on": "On",
      "off": "Off",
      "maxQuantity": "Max Quantity",
      "maxQuantityDesc": "Max 100, min 1",
      "unitPiece": "pcs",
      "theme": "Theme",
      "themeDesc": "UI theme",
      "selectLight": "Light",
  ...
  "fields": [
    ...
    },
    {
      ...
      "key": "activated",
      "nameI18nKey": "enabled",
      "descriptionI18nKey": "enabledDesc",
      "trueTextI18nKey": "on",
      "falseTextI18nKey": "off",
      "defaultValue": true
    },
    {
      "fieldType": "integer",
      "key": "maxCount",
      "nameI18nKey": "maxQuantity",
      "descriptionI18nKey": "maxQuantityDesc",
      "unitI18nKey": "unitPiece",
      "lowerBound": 1,
      "upperBound": 100,
      "defaultValue": 10
    },
    {
      ...
      "key": "theme",
      "nameI18nKey": "theme",
      "descriptionI18nKey": "themeDesc",
      "valueType": "string",
      "multiple": false,
      "defaultValue": "light",
      "options": [
        {
          "nameI18nKey": "selectLight",
          "value": "light"
        },
        {
          "nameI18nKey": "selectDark",
          "value": "dark"
        }
      ]
    }
...
- 优先使用 `*I18nKey` 引用 `lang` 中的文案；只有临时或不需要多语言时才使用 `name`、`description`、`unit`、`placeholder` 等硬编码字段。

- 数值类型建议按需配置 `lowerBound` 和 `upperBound`，框架会据此校验 App 输入。

- `defaultValue` 必须和字段类型一致：`boolean` 用布尔值，`integer` / `float` 用数字，`string` 用字符串，`select` 按 `valueType` 和 `multiple` 决定单值或数组。

- `divider` 只负责分组展示，不要配置 `key`，也不能通过参数管理器读写。

- 代码中通过 `(name, value)` 写入时，`name` 必须对应 schema 中的 `key`，否则无法完成类型推断。
